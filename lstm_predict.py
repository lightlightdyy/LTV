...
data_lst = data[['finish_order_num','pas_complaint_dri_nums','sum_coupon_spend',
                'cancel_before_nums','sum_gmv']].applymap(func)
...
for i in range(X_train0.shape[0]):
    ...
    for j in range(len(tmp[0])):
        a=np.array(list((tmp[0][j],tmp[1][j],tmp[2][j],tmp[3][j],tmp[4][j])))
        train=np.append(train,a)
print(len(train))  #54600

X_train_tmp = train.reshape(X_train0.shape[0],DAYSINC,fe_num)
print(X_train_tmp.shape) #(350, 52, 3)
...
print(X_train.shape,X_test.shape) #(350, 51, 3) (150, 51, 3)
...
print(y_train.shape,y_test.shape) #(350, 1) (150, 1)
...
def LSTM_RNN(_X, _weights, _biases):
    ...
    lstm_cell_1 = tf.contrib.rnn.BasicLSTMCell(n_hidden, forget_bias=1.0, state_is_tuple=True) #不遗忘
...
plt.figure()
plt.plot(list(range(len(predictions))), predictions, color='b')
